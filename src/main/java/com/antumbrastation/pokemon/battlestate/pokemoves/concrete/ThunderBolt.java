...
public class ThunderBolt extends SpecialMove {
    ...
    public ThunderBolt(boolean playerOne) {
        super(playerOne, 90, 100, 0, Pokemon.PokeType.Electric);
        setName("ThunderBolt");
    }

    @Override
    public PokeMove duplicate() {
        return duplicate(new ThunderBolt(true));
    }
    ...
    public void debuff(Pokemon victim, BattleState state) {
        ...
        if (roll == 0 && victim.getMainStatus() == Pokemon.MainStatus.Fine) {
            victim.setMainStatus(Pokemon.MainStatus.Paralysis);
        }
    }
}
